...
import { GraduationCap, Award, Calendar, BookOpen, Plus, X } from 'lucide-react';

const Trainings = () => {
  const { data, loading, refreshData } = useData();
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ titre: '', departement: 'Comptabilité', date: '', participants: 0 });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await refreshData();
      setShowForm(false);
      setForm({ titre: '', departement: 'Comptabilité', date: '', participants: 0 });
    } catch {
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {trainings.map((t) => (
          <div key={t.id} className="bg-white rounded-[2.5rem] p-8 border border-ci-border shadow-sm hover:shadow-xl transition-all group">
            ...
            <div className="mt-8 pt-8 border-t border-ci-bg flex justify-between items-center">
                <select
                  value={t.statut}
                  onChange={(e) => handleStatusChange(t.id, e.target.value)}
                  className="text-[10px] font-black text-ci-green uppercase bg-ci-bg px-3 py-2 rounded-xl border-none outline-none cursor-pointer"
                >
        ...
        <div className="fixed inset-0 bg-ci-dark/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-lg p-10 shadow-2xl animate-scaleIn">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-ci-muted">Titre</label>
                <input required value={form.titre} onChange={(e) => setForm({ ...form, titre: e.target.value })}
                  className="w-full mt-2 px-4 py-3 bg-ci-bg rounded-xl font-bold outline-none" placeholder="Ex: Gestion de la Paie CI" />
              </div>
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-ci-muted">Département</label>
                <select value={form.departement} onChange={(e) => setForm({ ...form, departement: e.target.value })}
                  ...
                  <option>Comptabilité</option><option>Sécurité</option><option>BTP</option><option>Pédagogie</option>
                </select>
              </div>
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-ci-muted">Date</label>
              ...
              <button type="submit" className="w-full py-4 bg-ci-green text-white rounded-2xl font-black text-xs uppercase tracking-widest">
